...
export default function Predict() {
  const API_URL = "https://my-projectml.onrender.com/predict";
  ...
  const stepTitles = ['Patient Information', 'Clinical Measurements', 'Lifestyle Factors'];
  const stepFields: Record<number, (keyof FormData)[]> = {
    0: ['age', 'gender', 'height', 'weight'],
    1: ['ap_hi', 'ap_lo', 'cholesterol', 'gluc'],
    2: ['smoke', 'alco', 'active'],
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (parseFloat(formData.weight) < 20 || parseFloat(formData.weight) > 300) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://my-projectml.onrender.com/predict', {
        method: 'POST',
        headers: {
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'Failed to connect to prediction service. Make sure the Flask API is running on port 5000.'
      );
    } finally {
      setLoading(false);
    }
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">Step {step + 1} of 3 — {stepTitles[step]}</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {step === 1 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="ap_lo">Diastolic Blood Pressure (mm Hg)</Label>
                    <Input
                      id="ap_lo"
                      type="number"
                      placeholder="e.g., 80"
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <Button type="submit" className="ml-auto" size="lg" disabled={loading}>
                  {loading ? (
                    <>
